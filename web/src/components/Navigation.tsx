    ...
    <>
      ...
      {isMenuOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-40 flex flex-col items-center justify-center gap-8 text-2xl font-medium text-gray-800 bg-white/80 dark:bg-black/80 backdrop-blur-lg dark:text-gray-200"
        >
          <a href="#docs" onClick={() => setIsMenuOpen(false)}>Docs</a>
          <a href="#playground" onClick={() => setIsMenuOpen(false)}>Playground</a>
          <a href="#dictionary" onClick={() => setIsMenuOpen(false)}>Dictionary</a>
        </motion.div>
      )}
    </>
  );
